...
\G Wait until a key is pressed and return the ASCII code
	...
	0<> UNTIL
	LD A, $5 (IX+)  \ Wait until non-zero code appears.
	OR A
      0<> UNTIL
    THEN
    LD $18 (IX+), $0     \ Clear the ASCII code.
    LD $5 (IX+), $0     \ Clear the ASCII code.
    POP IX
    PUSH DE     
    LD DE, $00
    LD E, A
...
\G Check if a key is pressed. Return a flag    
    PUSH IX
    LD A, $8
    RST .LIL $8 \ Get index to sysvars.
...
VARIABLE CONTEXT 7 CELLS-T ALLOT-T ( --- a-addr)
